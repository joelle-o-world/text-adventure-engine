  ...
  if (table) {
    ...
    for (let statement of table.involving(e)) {
      ...
      for (let syntax of ctx.linkingMatrix.meaningToSyntaxs(meaning)) {
        // Handle the syntax
        if (syntax instanceof Noun) nouns.push(syntax.str);
        else if (syntax instanceof Adjective) adjectives.push(syntax.str);
        ...
        } else throw `Unexpected syntax type: ${syntax}`;
      }
    }

    return { nouns, adjectives, sentences };
  } else throw `collectEntitySyntaxs expects context to have a truth table.`;
}
...
export function composeEntity(
  ...
  if (useProperNoun) {
    ...
    for (let properNoun in ctx.properNouns)
      if (ctx.properNouns[properNoun] == e) properNouns.push(properNoun);
  ...
  if (numberOfEmbeddedSentences > 0 && sentences.length > 0) {
    if (numberOfEmbeddedSentences > 1)
      console.warn(
        "Using more than 1 embedded sentence in a noun phrase leads to strange results."
      );
