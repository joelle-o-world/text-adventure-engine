  ...
  for (let statement of claim.iterate()) {
    ...
    for (let a of partialMappings) {
      for (let b of accumulatedMappings) {
        ...
        if (c) combinedMappings.push(c);
      }
    }

    if (combinedMappings.length == 0) return null;
...
export function findCompleteMappings<TruthValue extends string>(
  ...
  if (mappings) {
    ...
    if (completeMappings.length) return completeMappings;
    else return null;
  } else return null;
}
...
export function combinePartialMappings(
  ...
  if (a.length == b.length) {
    ...
    for (let i in a)
      if (a[i] == b[i] || b[i] === null) c[i] = a[i];
      else if (a[i] == null) c[i] = b[i];
      else return null;
    return c;
  } else return null;
}
...
export function compareMappings(a: PartialMapping, b: PartialMapping) {
...
export function findImperfectMappings<TruthValue extends string>(
  ...
  const update = (
    ...
    if (!set[symbol]) set[symbol] = { mapping, score };
    else set[symbol].score = Math.max(score, set[symbol].score);
  };
  ...
  for (let statement of claim.iterate()) {
    ...
    for (let M of mapFromSingleSentence(
      claim.variables,
      statement,
      onto,
      given
  ...
  return Object.values(accumulation).sort((a, b) => b.score - a.score);
}
...
export function* mapFromSingleSentence<TruthValue extends string>(
  ...
  const constants = sentence.args.map((arg) => {
    ...
    if (variableIndex != -1) {
      if (given[variableIndex]) return given[variableIndex];
      else return null;
    } else return arg;
  });

  const varPositions = identifyVarPositions(variables, sentence);
  const duplicatePositions: number[][] = varPositions.filter(
    (arr) => arr !== null && arr.length > 1
  ) as number[][];
      ...
      if (
        ...
        duplicatePositions.every((indexs) => {
          for (let i = 1; i < indexs.length; ++i)
            if (fact.sentence.args[0] != fact.sentence.args[i]) return false;
          // Otherwise
          return true;
        })
      )
        // Yield a partial mapping
        yield varPositions.map((indexs) =>
          indexs ? fact.sentence.args[indexs[0]] : null
        );
    }
  }
}
...
export function identifyVarPositions(
  ...
  return variables.map((x) => {
    ...
    for (let i = 0; i < sentence.args.length; ++i)
      if (sentence.args[i] == x) list.push(i);
