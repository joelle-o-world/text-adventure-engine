...
export function parseVariableTable(str: string) {
  const reg =
    /(there exists|∃) \(?(?<vars>.+)\)? (s\.t\.|such that) (?<body>{.+})/is;
...
export function quickTruthTable(src: string): TruthTable<string> {
  ...
  else throw `Invalid truth table source: '${src}'`;
}
...
export function quickVariableTable(src: string): VariableTable<string> {
  ...
  if (table) return table;
  else throw `Invalid VariableTable src: '${src}'`;
}

export function quickAdditionTable(
  parent: TruthTable | string,
  addition: TruthTable | string
) {
