...
> extends TruthTable<TruthValue> {
  ...
  constructor(...variables: Variable[]) {
    super();
    this.variables = [];
    if (variables.length) this.addVariables(...variables);
  }
  ...
  addVariables(...vars: Variable[]) {
    for (let v of vars) this.addVariable(v);
  }
  ...
  findMappingErrors(table: TruthTable<TruthValue>, mapping: Entity[]) {
    ...
    for (let { sentence, truth } of mapped) {
      if (table.lookUp(sentence) != truth) errors.push(sentence);
    }
  ...
  testMapping(table: TruthTable<TruthValue>, mapping: Entity[]) {
    ...
    for (let { sentence, truth } of mapped)
      if (table.lookUp(sentence) != truth) return false;
  ...
  get symbol() {
    return `∃ ${this.variables.join(",")} s.t. ${this.symbolBody}`;
  }
  ...
  merge(...tables: (TruthTable<TruthValue> | VariableTable<TruthValue>)[]) {
    for (let table of tables)
      if (table instanceof VariableTable) this.addVariables(...table.variables);
