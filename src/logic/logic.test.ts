...
test("Truth table identity", () => {
  ...
  expect(
    stringifySentence(table.idMapSentence({ predicate: P, args: [a, b] }))
  ...
  expect(table.lookUp({ predicate: P, args: [a, b] })).toBe("true");
  expect(table.lookUp({ predicate: P, args: [d, b] })).toBe(
    table.defaultTruthValue
  );

  table.makeIdentical(d, a, false);
  expect(table.lookUp({ predicate: P, args: [a, b] })).toBe(
    table.defaultTruthValue
  );
...
test("VariableTable", () => {
  ...
  expect(subbed.lookUp({ predicate: P, args: [x] })).toBe(
    subbed.defaultTruthValue
  );
  expect(subbed.lookUp({ predicate: Q, args: [y] })).toBe(
    subbed.defaultTruthValue
  );
  expect(subbed.lookUp({ predicate: P, args: [a] })).toBe("true");
  expect(subbed.lookUp({ predicate: P, args: [b] })).toBe(
    subbed.defaultTruthValue
  );
  expect(subbed.lookUp({ predicate: Q, args: [a] })).toBe(
    subbed.defaultTruthValue
  );
  ...
  expect(varTable.lookUp({ predicate: P, args: [a] })).toBe(
    varTable.defaultTruthValue
  );
  expect(varTable.lookUp({ predicate: Q, args: [b] })).toBe(
    varTable.defaultTruthValue
  );
});
...
test("VariableTable::findMappingErrors", () => {
  ...
  expect(hypothesis.findMappingErrors(theTruth, [a, b])).toBe(null);
  ...
  expect(hypothesis.findMappingErrors(theTruth, [a, b])).toStrictEqual([
    { predicate: P, args: [b] },
  ]);
  expect(hypothesis.testMapping(theTruth, [a, b])).toBe(false);
  expect(hypothesis.findMappingErrors(theTruth, [b, a])).toHaveLength(2);
...
test("Merging VariableTables", () => {
  ...
  expect(table2.lookUp({ predicate: P, args: [x, y] })).toBe(
    table2.defaultTruthValue
  );
});
