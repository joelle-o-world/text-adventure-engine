...
export class AdditionTable<
  TruthValue extends string
> extends TruthTable<TruthValue> {
  parent: TruthTable<TruthValue> | null;
  constructor(
    parent?: TruthTable<TruthValue> | null,
    addition?: TruthTable<TruthValue>
  ) {
    super();
    this.parent = parent || null;
    if (addition) this.eat(addition);
  }
  ...
  *byPredicate(P: Predicate) {
    ...
    if (this.parent)
      ...
      for (let assignment of this.parent.byPredicate(P))
        if (!subIndex[stringifyArgs(assignment.sentence)]) yield assignment;
  }

  *iterateAddition() {
    for (let P in this.index)
      for (let args in this.index[P]) yield this.index[P][args];
  }
  ...
  toString() {
    ...
    if (this.parent) return `${this.parent.symbol} & ${str}`;
    else return str;
  }
  ...
  idMapEntity(e: Entity): Entity {
    while (true) {
      if (this.identityMap[e]) e = this.identityMap[e];
      else if (this.parent && e != (e = this.parent.idMapEntity(e))) continue;
      else return e;
    }
  }
  ...
  flatten(): TruthTable<TruthValue> {
    if (this.parent) {
      ...
      let table =
        this.parent instanceof AdditionTable
          ? this.parent.flatten()
          : this.parent.clone();
  ...
  mergeToParent(): void {
    ...
    else throw "No Parent to merge into!";
  }
