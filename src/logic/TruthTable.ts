...
export class TruthTable<TruthValue extends string = "T" | "F" | "?"> {
  ...
  assign(sentence: Sentence | string, truth: TruthValue | "?"): this {
    ...
    if (!this.index[predicateSymbol])
      ...
      else this.index[predicateSymbol][argsSymbol] = { sentence, truth };
    }
  ...
  remove(sentence: Sentence | string, truth?: TruthValue): void {
    ...
    if (this.index[predicate]) {
      ...
      else delete this.index[predicate][args];
    }
  }
  ...
  lookUp(sentence: Sentence | string): TruthValue | "?" {
    ...
    else return this.defaultTruthValue;
  }
  ...
  *iterate() {
    for (let P in this.index)
      for (let assignment of this.byPredicate(P)) yield assignment;
  }
  ...
  *byPredicate(P: Predicate) {
    ...
    for (let i in subIndex) yield subIndex[i];
  }
  ...
  *involving(e: Entity) {
    ...
    for (let statement of this.iterate())
      if (statement.sentence.args.includes(e)) yield statement;
  }
  ...
  toString() {
    return (
      ...
      this.facts
        .map(
          ({ sentence, truth }) => `\t${stringifySentence(sentence)}=${truth}`
        )
        .join("\t&\n") +
      "\n}"
    );
  ...
  get entities(): Entity[] {
    ...
    for (let { sentence } of this.iterate())
      for (let arg of sentence.args) if (!list.includes(arg)) list.push(arg);
  ...
  makeIdentical(a: Entity, b: Entity, addToMap = true) {
    ...
    if (addToMap) this.identityMap[b] = a;
  }
  ...
  idMapEntity(e: Entity): Entity {
    while (this.identityMap[e]) e = this.identityMap[e];
  ...
  measureCompatibility(b: TruthTable) {
    ...
    for (let { sentence, truth } of b.iterate()) {
      ...
      if (truth2 == INDIFFERENT) ++introductions;
      else if (truth2 == truth) ++overlap;
      else ++contradictions;
    }
  ...
  hasContradictionsWith(table: TruthTable) {
    const [larger, smaller] =
      this.length > table.length ? [this, table] : [table, this];

    for (let { sentence, truth } of smaller.iterate()) {
      let truth2 = larger.lookUp(sentence);
      if (truth2 != INDIFFERENT && truth2 != truth) return true;
    }
  ...
  static merge(...tables: TruthTable[]) {
    ...
    for (let table of tables) combined.merge(table);
