...
test("IfThenRule", () => {
  ...
  let myRule = new IfThenRule(
    ...
    new VariableTable(x, y, z, x1).assign(
      { predicate: Q, args: [x, y, z, x1] },
      "T"
    )
  );
  ...
  if (mappings.length) {
    expect(mappings).toStrictEqual([[a, b, c, d]]);
  } else fail();

  let [...consequences] = myRule.additionConsequences(addition, table);
  expect(consequences[0].lookUp({ predicate: Q, args: [a, b, c, d] })).toBe(
    "T"
  );
});
...
test("RuleSet", () => {
  ...
  let table = new TruthTable().T(P, c, d).T(R, c, d);

  const addition = new TruthTable().T(P, a, b).T(R, a, b);

  let [...consequences] = rules.additionConsequences(addition, table);
