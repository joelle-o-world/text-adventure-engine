...
export class IfThenRule<TruthValue extends string = "T" | "F" | "?"> {
  ...
  constructor(
    ...
    if (antecedent.numberOfVariables != consequent.numberOfVariables)
      throw new Error(
        "Antecedent and consequent have different number of variables"
      );
  ...
  *additionMappings(
    addition:
      ...
      | { sentence: Sentence; truth: TruthValue },
    onto: TruthTable<TruthValue>
  ) {
    let additionTable =
      addition instanceof TruthTable
        ? addition
        : new TruthTable<TruthValue>().assign(
            addition.sentence,
            addition.truth
          );
    ...
    for (let statement of this.antecedent.iterate()) {
      ...
      for (let initialMapping of initialMappings) {
        ...
        if (mappings) for (let mapping of mappings) yield mapping;
      }
    }
  }
  ...
  *additionConsequences(
    ...
    for (let mapping of this.additionMappings(addition, onto))
      yield this.consequent
        ...
        .filter(
          ({ sentence, truth }) =>
            onto.lookUp(sentence) != truth && addition.lookUp(sentence) != truth
        );
  }
