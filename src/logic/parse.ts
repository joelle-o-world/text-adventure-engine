...
export function parseArgs(str: string) {
  ...
  if (args.every((arg) => isEntity(arg))) return args;
  else return null;
}
...
export function parseSentence(str: string) {
  ...
  if (parse) {
    ...
    if (args && isPredicate(predicate)) return { args, predicate };
    else return null;
  } else return null;
}
...
export function quickSentence(str: string) {
  ...
  else throw "Unable to parse logical sentence: " + str;
}
...
export function parseAssignment(str: string) {
  ...
  if (sentence && truth) return { sentence, truth };
  else return null;
}
