 ...
 * Language Processing.
 */
export function cfgRecognise(
  grammar: ContextFreeGrammar<string>,
  str: string[]
): boolean {
  ...
  const tableIncludes = (a: number, b: string, c: number) =>
    table.some(([x, y, z]) => x == a && b == y && c == z);
  ...
  if (
    ...
    tableIncludes(0, grammar.startingSymbol, str.length)
  )
    return true;
  else return false;
...
export function getNonTerminalSubstitutions(
  grammar: ContextFreeGrammar<any>,
  nonTerminalSymbol: any,
  compareSymbol = Object.is
) {
  return grammar.nonTerminalRules.filter((rule) =>
    compareSymbol(rule.head, nonTerminalSymbol)
  );
  //.map(rule => rule.body);
}

export function getTerminalSubstitutions(
  grammar: ContextFreeGrammar<any>,
  nonTerminalSymbol: any,
  compareSymbol = Object.is
) {
  return grammar.terminalRules.filter((rule) =>
    compareSymbol(rule.head, nonTerminalSymbol)
  );
  //.map(rule => rule.body)
}

export function* recursiveSubstitutions(
  grammar: ContextFreeGrammar<any>,
  S = grammar.startingSymbol,
  compareSymbol = Object.is
): Generator<any[]> {
  // Yield through terminal substitutions for S
  const terminalSubstitutions = getTerminalSubstitutions(
    grammar,
    S,
    compareSymbol
  );
  for (let rule of terminalSubstitutions) yield [rule.body];

  const nonTerminalSubstitutions = getNonTerminalSubstitutions(
    grammar,
    S,
    compareSymbol
  );
...
export function cleanHiddenAnnotations(
  ...
  for (let item of body) {
    if (typeof item == "object" && item.head && item.body) {
      ...
      else newBody.push(cleanHiddenAnnotations(item, isHidden));
    }
    // item is a terminal (?) symbol
    else newBody.push(item);
  }

  return { head, body: newBody, ruleId };
}

export function stringifyAnnotation(
  annotation: AnnotatedTree | string
): string {
...
export function randomSubstitution(
  ...
  if (
    Math.random() *
      (terminalSubstitutions.length + nonTerminalSubstitutions.length) <
    terminalSubstitutions.length
  )
    // Return a terminal substitution
    return [
      terminalSubstitutions[
        Math.floor(Math.random() * terminalSubstitutions.length)
      ].body,
    ];
  else {
    let sub =
      nonTerminalSubstitutions[
        Math.floor(Math.random() * nonTerminalSubstitutions.length)
      ].body;
    return [
