...
describe("PredicateSyntax predicates", () => {
  test.each([
    ...
    'createPredicateSyntaxPredicate("%s") = %s',
    (syntax: { verb: string; params: string[] }, p) => {
      expect(createPredicateSyntaxPredicate(syntax)).toBe(p);
    }
  );

  describe("createPredicateSyntaxPredicate() produces valid predicates", () => {
    const syntaxs = [{ verb: "eat", params: ["subject", "object"] }];
    const predicates = syntaxs.map((syntax) =>
      createPredicateSyntaxPredicate(syntax)
    );
