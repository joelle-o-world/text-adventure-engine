...
export function interpretNounPhraseParse(np: NounPhraseParse) {
  ...
  else throw `Unexpected syntax kind: '${(np as NounPhraseParse).kind}'`;
}

export function interpretSimplNounPhraseParse(np: SimpleNounPhraseParse): {
  table: VariableTable;
  returning: Entity;
} {
  ...
  for (let adj of np.adjectives)
    table.assign({ predicate: createAdjectivePredicate(adj), args: [e] }, "T");
