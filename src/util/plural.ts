...
export function toPlural(singularNoun: string): string {
  ...
  if (irregular[singularNoun]) return irregular[singularNoun];

  // If the singular noun ends in -o, ‑s, -ss, -sh, -ch, -x, or -z, add ‑es
  if (/(o|s|ss|sh|ch|x|z)$/i.test(singularNoun)) return singularNoun + "es";
  ...
  if (/us$/.test(singularNoun)) return singularNoun.slice(0, -1) + "i";
  ...
  if (/on$/.test(singularNoun)) return singularNoun.slice(0, -2) + "a";
...
export function toSingular(pluralNoun: string): string | null {
  ...
  if (irregularInverted[pluralNoun]) return irregularInverted[pluralNoun];

  // If the plural noun ends -ies, replace with -y
  if (/ies$/.test(pluralNoun)) return pluralNoun.slice(0, -3) + "y";
  ...
  if (/es$/.test(pluralNoun)) return pluralNoun.slice(0, -2);

  // If the plural noun ends -s, remove -s
  if (/s$/.test(pluralNoun)) return pluralNoun.slice(0, -1);

  // If the plural noun ends -i, replace with -us
  if (/i$/.test(pluralNoun)) return pluralNoun.slice(0, -1) + "us";

  // If the plural noun ends -a, replace with -on
  if (/a$/.test(pluralNoun)) return pluralNoun.slice(0, -1) + "on";
