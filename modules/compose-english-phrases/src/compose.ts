...
export default function compose(args: SentenceComposable): string {
  ...
  if (!subject) throw "Cannot construct sentence without subject.";
  ...
  if (question && tense == "simple_past") {
    ...
    if (nounPhraseFor == "subject") verbPhrase = `${subject} which ${verb}`;
    ...
    verbPhrase = new Template(`_ ${nounPhraseFor} which _ <${verb}`).str([
      args[nounPhraseFor] as string,
      subject,
    ]);
  else verbPhrase = new Template(`_ <${verb}`).str([subject]);

  let toConcat = [verbPhrase];

  // Append object, if exists.
  if (object && nounPhraseFor != "object") toConcat.push(object);

  // Append any prepositional phrases.
  for (let prep in prepositions)
    if (prep != nounPhraseFor) toConcat.push(prep, prepositions[prep]);
