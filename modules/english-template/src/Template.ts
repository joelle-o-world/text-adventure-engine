...
const { getPerson, conjugate, anyPersonRegex } = alternativeConjugation;
import { Tense } from "english-transformations";
import { possessiveAdjectiveRegex } from "useful-regular-expressions";
...
export default class Template {
  ...
  str(args: string[]) {
    ...
    // Prepare arguments
    args = args.map((arg, i) => {
      if (this.params[i].possessive) return toPossessiveAdjective(arg);
      else return arg;
    });
  ...
  regex() {
    let argRegexs = this.params
      .map((arg) => {
        if (arg.possessive) return possessiveAdjectiveRegex.source;
        else if (arg.number) return "-?[\\.0-9]+";
        else return "[\\w ]+";
      })
      .map((str) => "(" + str + ")");
    let fluff = this.template.split(placeholderRegex);
  ...
  parse(str: string) {
    ...
    if (parse) {
      ...
      for (let i in this.params)
        if (this.params[i].number) args[i] = parseFloat(args[i] as string);
      return {
        args,
        syntax: this,
        tense: this.tense,
...
function handleConjugation(str: string, left?: string, right?: string) {
  ...
  for (let i = 0; i < verbs.length; ++i) {
    ...
    } else throw `Couldn't handle conjugation: (${left})${str}(${right})`;
