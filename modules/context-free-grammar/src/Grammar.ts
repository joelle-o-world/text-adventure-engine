...
import {
  ...
  ParseForest,
  ParseTable,
  ParseTreeNonTerminalSymbol,
  ParseTreeTerminalSymbol,
  TerminalRule,
  TypeAssertions,
} from "./types";
...
export interface GrammarConstructorOptions<
  TerminalSymbol,
...
> {
   ...
   */
  compareSymbol?: (
    a: TerminalSymbol | NonTerminalSymbol,
    b: TerminalSymbol | NonTerminalSymbol
  ) => boolean;
...
> {
  ...
  startingSymbol: NonTerminalSymbol;
  compareSymbol: (
    a: NonTerminalSymbol | TerminalSymbol,
    b: NonTerminalSymbol | TerminalSymbol
  ) => boolean;
  isHidden: (S: NonTerminalSymbol) => boolean;
  beQuiet: boolean;
  ...
  constructor({
    ...
    // @ts-ignore
    isTerminalSymbol = () => {
      throw "No type guard defined for terminal symbols.";
    },
    // @ts-ignore
    isNonTerminalSymbol = () => {
      throw "No type guard defined for non-terminal symbols.";
    },
    ...
    if (!startingSymbol) {
      ...
      if (topSymbols.length) {
        if (topSymbols.length == 1) this.startingSymbol = topSymbols[0];
        else if (!this.beQuiet)
          console.warn(
            "Cannot determine `startingSymbol` of grammar. Candidates:",
            topSymbols
          );
      } else if (!this.beQuiet)
        console.warn(
          "Cannot dermine `startingSymbol` of grammar. No candidates."
        );
    } else this.startingSymbol = startingSymbol;
  }
  ...
  recognise(
    ...
    const tableIncludes = (a: number, b: NonTerminalSymbol, c: number) =>
      table.some(([x, y, z]) => x == a && this.compareSymbol(b, y) && c == z);
    ...
    const addToTable = (i: number, B: NonTerminalSymbol, j: number) => {
      ...
      for (let { head, body } of this.aliasRules)
        if (this.compareSymbol(body, B)) addToTable(i, head, j);
    };

    // Initialise the table if passed an `initialTable` arg
    if (initialTable) for (let [i, S, j] of initialTable) addToTable(i, S, j);
      ...
      for (let i = j - 1; i >= 0; --i) {
        for (let k = i + 1; k <= j - 1; ++k)
          ...
          for (let rule of this.nonTerminalRules)
            if (
              tableIncludes(i, rule.body[0], k) &&
              tableIncludes(k, rule.body[1], j)
            )
              // Add a record to the table
          ...
          for (let rule of this.nonTerminalRules)
            if (
              ...
              nonTerminalRules.push({
                ...
                body: [
                  { from: i, S: rule.body[0], to: k },
                  { from: k, S: rule.body[1], to: j },
                ],
                F: rule.F,
              });
            }
  ...
  *recursiveSubstitutions(
  ...
  randomSubstitution(S = this.startingSymbol): TerminalSymbol[] {
    ...
    const total =
      terminalRules.length + nonTerminalRules.length + aliasRules.length;
    ...
    if (n < terminalRules.length)
      ...
      return [...this.randomSubstitution(a), ...this.randomSubstitution(b)];
    } else {
      // n landed on an alias rule
      let { body } =
        aliasRules[n - terminalRules.length - nonTerminalRules.length];
  ...
  *recursiveTrees(
    S = this.startingSymbol
  ): Generator<Tree<TerminalSymbol, NonTerminalSymbol>> {
    const isRelevant = (rule: AnyRule<TerminalSymbol, NonTerminalSymbol>) =>
      this.compareSymbol(rule.head, S);
  ...
  stringifyTree(
    tree: Tree<TerminalSymbol, NonTerminalSymbol, any, any>
  ): string {
    if (tree.ruleKind == "terminal") {
      if (this.isHidden(tree.head))
        ...
        return `[${this.stringifyNonTerminalSymbol(
          tree.head
        )} ${this.stringifyTerminalSymbol(tree.body)}]`;
    } else if (tree.ruleKind == "nonTerminal") {
      if (this.isHidden(tree.head))
        return `${this.stringifyTree(tree.body[0])} ${this.stringifyTree(
          tree.body[1]
        )}`;
      else
        return `[${this.stringifyNonTerminalSymbol(
          tree.head
        )} ${this.stringifyTree(tree.body[0])} ${this.stringifyTree(
          tree.body[1]
        )}]`;
    } else if (tree.ruleKind == "alias") {
      if (this.isHidden(tree.head)) return this.stringifyTree(tree.body);
      else
        return `[${this.stringifyNonTerminalSymbol(
          tree.head
        )} ${this.stringifyTree(tree.body)}]`;
    }
    // @ts-ignore
    else throw `Unexpected rule kind in tree: ${tree.ruleKind}`;
  }
  ...
  compareTrees(
    ...
    if (oak.ruleKind == "terminal")
      return (
        cedar.ruleKind == "terminal" &&
        this.compareSymbol(oak.head, cedar.head) &&
        this.compareSymbol(oak.body, cedar.body)
      );
    else if (oak.ruleKind == "nonTerminal")
      return (
        cedar.ruleKind == "nonTerminal" &&
        this.compareSymbol(oak.head, cedar.head) &&
        this.compareTrees(oak.body[0], cedar.body[0]) &&
        this.compareTrees(oak.body[1], cedar.body[1])
      );
    else if (oak.ruleKind == "alias")
      return (
        cedar.ruleKind == "alias" &&
        this.compareSymbol(oak.head, cedar.head) &&
        this.compareTrees(oak.body, cedar.body)
      );
    else throw "Tree has unexpected rule kind: " + JSON.stringify(oak);
  }

  *iterateRules() {
    for (let rule of this.terminalRules) yield rule;
    for (let rule of this.nonTerminalRules) yield rule;
    for (let rule of this.aliasRules) yield rule;
  }

  listAllNonTerminals() {
    const alphabet: NonTerminalSymbol[] = [];
    const add = (S: NonTerminalSymbol) => {
      if (!alphabet.some((H) => this.compareSymbol(H, S))) alphabet.push(S);
    };

    for (let { head } of this.terminalRules) add(head);
  ...
  listTopNonTerminals(): NonTerminalSymbol[] {
    return this.listAllNonTerminals().filter((S) =>
      this.nonTerminalRules.every(
        (rule) =>
    ...
    for (let i = 0; i < this.terminalRules.length; ++i) {
      ...
      for (let j = i + 1; j < this.terminalRules.length; ++j) {
        ...
        if (this.compareTerminalRules(a, b)) {
          if (a.F == b.F)
            throw `Grammar has duplicate TERMINAL rules with different evaluation functions. ${this.stringifyTerminalRule(
              a
            )}`;
          else
            throw `Grammar has duplicate TERMINAL rule: ${this.stringifyTerminalRule(
              a
            )}`;
        }
      }
    ...
    for (let i = 0; i < this.nonTerminalRules.length; ++i) {
      ...
      for (let j = i + 1; j < this.nonTerminalRules.length; ++j) {
        ...
        if (this.compareNonTerminalRules(a, b)) {
          if (a.F == b.F)
            throw `Grammar has duplicate NON-TERMINAL rules with different evaluation functions: ${this.stringifyNonTerminalRule(
              a
            )}`;
          else
            throw `Grammar has duplicate NON-TERMINAL rules: ${this.stringifyNonTerminalRule(
              a
            )}`;
        }
      }
    ...
    for (let i = 0; i < this.aliasRules.length; ++i) {
      ...
      for (let j = i + 1; j < this.aliasRules.length; ++j) {
        ...
        if (this.compareAliasRules(a, b)) {
          if (a.F == b.F)
            throw `Grammar has duplicate ALIAS rules with different evaluation functions: ${this.stringifyAliasRule(
              a
            )} \n\n\tFirst function: ${a.F.toString()}\n\n\tSecondFunction: ${b.F.toString()}`;
          else
            throw `Grammar has duplicate ALIAS rules: ${this.stringifyAliasRule(
              a
            )}`;
        }
      }
   ...
   */
  compareTerminalRules(
    a: TerminalRule<TerminalSymbol, NonTerminalSymbol>,
    b: TerminalRule<TerminalSymbol, NonTerminalSymbol>
  ): boolean {
    return (
      this.compareSymbol(a.head, b.head) && this.compareSymbol(a.body, b.body)
    );
  ...
  compareNonTerminalRules(
    ...
    return (
      this.compareSymbol(a.head, b.head) &&
  ...
  stringifyNonTerminalRule(
    rule: NonTerminalRule<TerminalSymbol, NonTerminalSymbol>
  ) {
    return `${this.stringifyNonTerminalSymbol(
      rule.head
    )} -> ${this.stringifyNonTerminalSymbol(
      rule.body[0]
  ...
  get numberOfRules() {
    return (
      this.terminalRules.length +
      this.nonTerminalRules.length +
      this.aliasRules.length
    );
  }
  ...
  checkRules() {
    if (!this.isNonTerminalSymbol(this.startingSymbol))
      throw (
        "Starting symbol is not valid: " + JSON.stringify(this.startingSymbol)
      );

    for (let rule of this.terminalRules)
      if (
        !isTerminalRule(rule, this.isTerminalSymbol, this.isNonTerminalSymbol)
      ) {
  ...
  static merge<T, NT>(...grammars: Grammar<T, NT>[]): Grammar<T, NT> {
    ...
    for (let g of grammars) {
      ...
      for (let S in g.typeAssertions) {
        if (typeAssertions[S]) throw `Type assertion conflict for '${S}'`;
