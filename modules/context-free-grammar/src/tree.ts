...
> =
  ...
  | {
      ...
      head: NT;
      body: [
        Tree<T, NT, GrammarTerminal, GrammarNonTerminal>,
        Tree<T, NT, GrammarTerminal, GrammarNonTerminal>
      ];
    }
  | {
...
export function evaluateTree<TerminalSymbol>(tree: Tree<TerminalSymbol>): any {
  if (tree.ruleKind == "terminal") return tree.rule.F(tree.body);
  ...
  } else throw "Could not evaluate tree: " + JSON.stringify(tree, null, 4);
}

export function flattenTree<TerminalSymbol>(
  tree: Tree<TerminalSymbol>
): TerminalSymbol[] {
  if (tree.ruleKind == "terminal") return [tree.body];
  else if (tree.ruleKind == "nonTerminal")
    return [...flattenTree(tree.body[0]), ...flattenTree(tree.body[1])];
  else if (tree.ruleKind == "alias") return flattenTree(tree.body);
  else throw "Cannot flatten tree because of invalid `ruleKind`: " + tree;
}
...
export function mapTree<T, NT, GT, GNT, T2, NT2>(
  ...
  else throw "Tree has unexpected rule-kind:" + JSON.stringify(tree);
}

export type ParseTree<T, NT, GT = T, GNT = NT> = Tree<
  ParseTreeTerminalSymbol<T, NT>,
  ParseTreeNonTerminalSymbol<T, NT>,
  GT,
  GNT
>;

export function rewindParseTreeSymbols<T, NT, GT, GNT>(
  parseTree: ParseTree<T, NT, GT, GNT>
): Tree<T, NT, GT, GNT> {
  return mapTree(
    parseTree,
    (S) => S.S,
