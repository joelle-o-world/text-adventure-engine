...
export class IndexedGrammar extends Grammar<string, string> {
  ...
  buildIndexes(): void {
    ...
    for (let rule of this.terminalRules) {
      const T = rule.body;
      if (this.terminalParseIndex[T]) this.terminalParseIndex[T].push(rule);
      else this.terminalParseIndex[T] = [rule];
    }
    ...
    for (let rule of this.nonTerminalRules) {
      ...
      if (this.nonTerminalParseIndex[B]) {
        ...
        else this.nonTerminalParseIndex[B][C] = [rule];
      } else this.nonTerminalParseIndex[B] = { [C]: [rule] };
    }
    ...
    for (let rule of this.aliasRules) {
      const B = rule.body;
      if (this.aliasParseIndex[B]) this.aliasParseIndex[B].push(rule);
      else this.aliasParseIndex[B] = [rule];
    }
  }
  ...
  parse(str: string[], initialTable?: ParseTable<string>) {
    ...
    const addToTable = (i: number, B: string, j: number) => {
      table.push([i, B, j]);
