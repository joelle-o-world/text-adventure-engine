...
describe("Testing Grammar", () => {
  ...
  test.skip("Grammar.prototype.recursiveSubstitutions", () => {
    ...
    for (let str of fishGrammar.recursiveSubstitutions())
      if (n++ > 25) break;
      else expect(fishGrammar.recognise(str)).toBe(true);
  });
  ...
  describe("Comparing Trees", () => {
    ...
    test.todo(
      "Comparison is positive when testing trees against deep clones of themseelves"
    );
      ...
      for (let i = 0; i < trees.length - 1; ++i) {
        ...
        for (let j = i + 1; j < trees.length; ++j) {
          ...
          if (str1 == str2)
            ...
            continue;
          else expect(fishGrammar.compareTrees(trees[i], trees[j])).toBe(false);
        }
      }
    });
  });
  ...
  describe("Evaluating Trees", () => {
    ...
    describe("Evaluating/recognising/parsing trees generated from a string grammar", () => {
      ...
      for (let tree of trees) {
        ...
        test(`Recognising string "${str.join(
          " "
        )}" as belonging to the grammar`, () => {
          expect(GrAYmmer.recognise(str)).toBe(true);
        });

        test(`(Re-)Parsing string "${str.join(
          " "
        )}" produces original tree`, () => {
