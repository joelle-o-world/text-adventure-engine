...
import { wholeWord } from "regular-expression-operations";
...
test("Constructing a predicate syntax", () => {
  ...
  expect(syntax.verbRegex.source).toBe(wholeWord("goes|go").source);
  expect(syntax.prepositions).toStrictEqual(["to"]);
  if (!syntax.prepositionRegex) fail();
  else expect(syntax.prepositionRegex.source).toBe(wholeWord("to").source);
});

test("Parsing using a PredicateSyntax", () => {
  const syntax = new PredicateSyntax("live", ["subject", "in"]);
  let parse = syntax.parseSpecific(
    "the boy lives in the dutch barge",
    "simple_present"
  );
...
test("Composing questions using PredicateSyntax", () => {
  ...
  let str = syntax.str(["the moose", "this hoose"], {
    tense: "simple_present",
    question: true,
  });
...
test("Parsing questions using PredicateSyntax", () => {
  const syntax = new PredicateSyntax("live", ["subject", "in"]);
  let parse = syntax.parseSpecific("does a moose live in this hoose", {
    tense: "simple_present",
    question: true,
  });
  ...
  expect(
    syntax2.parseSpecific("is a moose aloose aboot this hoose", {
      tense: "simple_present",
      question: true,
    })
  ).toMatchObject({
...
test("Parsing questions & negatives using PredicateSyntax", () => {
  const syntax = new PredicateSyntax("live", ["subject", "in"]);
  let parse = syntax.parseSpecific("a moose does not live in this hoose", {
    tense: "simple_present",
    question: false,
    negative: "not",
  });
  ...
  const syntax2 = new PredicateSyntax("be aloose", ["subject", "aboot"]);
  expect(
    syntax2.parseSpecific("will a moose not be aloose aboot this hoose", {
      tense: "simple_future",
      question: true,
      negative: "not",
    })
...
test("Parsing/compose bijection", () => {
  ...
  for (let { syntax, args } of sentences)
    for (let tense of allTenses) {
      ...
      let qnParse = syntax.parseSpecific(qn, {
        tense,
        negative: "not",
        question: true,
      });
      ...
      for (let param of syntax.params) {
        ...
        let npnParse = syntax.parseSpecific(npn, {
          tense,
          nounPhraseFor,
          negative: "not",
        });
...
test("PredicateSyntax: Re-using indexed regular expressions", () => {
  ...
  let reg1 = syntax.composeVerbPhraseRegex({
    tense: "simple_present",
    negative: "not",
    question: true,
    nounPhraseFor: null,
  });
  let reg2 = syntax.composeVerbPhraseRegex({
    tense: "simple_present",
    negative: "not",
    question: true,
    nounPhraseFor: null,
