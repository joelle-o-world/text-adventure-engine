...
import { MorphologyRelation } from "english-morphology";
import { VerbTenseParse, assertIsVerbTenseParse } from "./parseTypings";

const VerbTenseGrammar = ContextFreeGrammar.quick(
  {
    "_SimplePresent -> _conjugatedVerb": ({ base }): VerbTenseParse => ({
      kind: "verbTense",
      verb: base,
      tense: "simplePresent",
    }),
    ...
    "_SimpleFuture -> will _infinitive": (verb: string): VerbTenseParse => ({
      kind: "verbTense",
      verb: verb,
      tense: "simpleFuture",
    }),
...
export default VerbTenseGrammar;
