...
import Grammar, { evaluateTree, ExamplePOSGrammar } from "context-free-grammar";
...
describe("Checking that grammar tree evaluations satisfy their type assertions", () => {
  describe.each([
    ...
    [
      ...
      Grammar.merge(PredicateSyntaxGrammar, ExamplePOSGrammar),
    ],
  ])(
    "%s's tree evaluations satisfy its type assertions",
    (name: string, G: Grammar) => {
      G.checkRules();
      G.assertNoDuplicateRules();

      describe.each(Object.keys(G.typeAssertions))(
        "Evaluating trees derived from '%s'",
        (S) => {
...
test("Known bug: stack overflow substituting _AdjList", () => {
  expect(() => {
    return Grammar.merge(NounPhraseGrammar, ExamplePOSGrammar)
      .recursiveTrees("_AdjList")
      .next();
  }).not.toThrowError();
});
