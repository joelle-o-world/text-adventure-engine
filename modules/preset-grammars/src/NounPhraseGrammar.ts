...
import {
  assertIsSimpleNounPhraseParse,
  assertIsAdjectiveParse,
  assertIsArticleParse,
  assertIsNounParse,
  SimpleNounPhraseParse,
  PronounParse,
  assertIsPronounParse,
  assertIsNounPhraseParse,
} from "./parseTypings";

const NounPhraseGrammar = ContextFreeGrammar.quick(
  "_article -> the|a|an|some|any",

  {
    "_AnyNoun -> _noun": (noun: string) => ({
      noun,
      couldBeSingular: true,
      couldBePlural: false,
    }),

    "_AnyNoun -> _pluralNoun": (noun) => ({
      noun,
      couldBePlural: true,
...

  {
    "_SimpleNounPhrase -> _article _AnyNoun": (
      article,
      { noun, couldBePlural, couldBeSingular }
    ): SimpleNounPhraseParse => ({
    ...
    "_SimpleNounPhrase -> _article _AdjList _AnyNoun": (
      article,
      adjectives,
      { noun, couldBeSingular, couldBePlural }
    ): SimpleNounPhraseParse => ({
      kind: "simpleNounPhrase",
      article,
      adjectives,
  ...
  {
    "_subjectPronoun -> I|you|she|he|they|it|we": (pronoun): PronounParse => ({
      kind: "pronoun",
      pronoun,
      couldBeSubject: true,
      couldBeObject: false,
    ...
    "_objectPronoun -> me|you|it|him|her|them|us": (pronoun): PronounParse => ({
      kind: "pronoun",
      pronoun,
...
export default NounPhraseGrammar;
